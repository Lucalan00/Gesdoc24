...
const beforeUpdateUsuario = async (user) => {
  try {
    ...
    if (user.contrasena && user.changed('contrasena')) {
      console.log('Nueva contraseña detectada, aplicando hash...');
      await hashPassword(user);
    }

    // Si el estado del usuario cambia, actualizamos la notificación relacionada
    if (user.changed('estado')) {
      console.log(`Actualizando estado del usuario a: ${user.estado}`);
      ...
      if (notificacion) {
        ...
        console.log('No se encontró una notificación para actualizar.');
      }
    }
  } catch (error) {
