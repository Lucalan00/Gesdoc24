...
const handleNotificationAction = async (req, res, estado) => {
  ...
  try {
    ...
    if (!notificacion) {
      ...
      console.log(' Notificación no encontrada. ID:', id_notificacion);
    ...
    if (!usuario) {
      await transaction.rollback();
      console.log(' Usuario asociado no encontrado en la notificación.');
    ...
    if (!req.userId || (req.userRol !== 'Administrador' && req.userRol !== 'Secretario')) {
      await transaction.rollback();
      console.log(' Permisos insuficientes para realizar la acción.');
      return res.status(403).json({ message: 'No tienes permisos para esta acción' });
    }

    //  **Actualizar estado del usuario**
    console.log('Actualizando estado del usuario:', usuario.id_usuario, 'Nuevo estado:', estado);
    ...
    console.log('Enviando correo a:', usuario.empleado.email);

    sendEmail(usuario.empleado.email, `Estado de tu registro: ${estado}`, mensaje)
      .then(() => console.log(` Correo enviado a ${usuario.empleado.email}`))
      .catch(error => console.error(' Error al enviar correo:', error));

    //  **Actualizar notificación**
    console.log('Marcando la notificación como leída:', id_notificacion);
    ...
    console.error(' Error al procesar la acción de notificación:', error);
