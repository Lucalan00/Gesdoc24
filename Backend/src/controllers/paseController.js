...
const createPase = async (req, res) => {
  ...
  const { id_doc, id_departamento_actual, id_departamento_destino, id_receptor, descripcion } = req.body;
  ...
  try {
    ...
    if (ultimoPase) {
      ...
      await ultimoPase.update({ 
        ...
        objetivo: descripcion || 'Traslado completado'
      }, { transaction: t });
    }
    const nuevoPase = await Pase.create({
      ...
      id_departamento_destino,
      id_receptor,
      objetivo: descripcion || 'En revisión',
    ...
    await t.rollback();
    console.error('Error en createPase:', error.message);
...
const getPasesByDocumento = async (req, res) => {
  try {
    ...
    console.error("Error en getPasesByDocumento:", error);
    res.status(500).json({ 
      success: false, 
...
const getAllPases = async (req, res) => {
  try {
    ...
    console.error("Error en getAllPases:", error);
    res.status(500).json({ 
      success: false, 
      message: "Error al recuperar los pases",
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
};
