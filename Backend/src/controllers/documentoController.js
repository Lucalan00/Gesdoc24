...
const deleteDocumento = async (req, res) => {
  try {
    // Log para depuración
    console.log('[deleteDocumento] req.userRol:', req.userRol, '| req.userId:', req.userId);
    const documentoToDelete = await documento.findByPk(req.params.id_doc);
    if (!documentoToDelete) {
      return res.status(404).json({ message: 'Documento no encontrado' });
    }

    await documentoToDelete.destroy();
